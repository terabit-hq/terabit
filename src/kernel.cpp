...
// Copyright (c) 2014 The Terabit developers
...
static bool CheckStakeKernelHashV2(CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (fPrintProofOfStake)
    {
        LogPrintf("CheckStakeKernelHashV2() : using modifier 0x%016x at height=%d timestamp=%s for block from timestamp=%s\n",
            nStakeModifier, nStakeModifierHeight,
            DateTimeStrFormat(nStakeModifierTime),
            DateTimeStrFormat(nTimeBlockFrom));
        LogPrintf("CheckStakeKernelHashV2() : check modifier=0x%016x nTimeBlockFrom=%u nTimeTxPrev=%u nPrevout=%u nTimeTx=%u hashProof=%s\n",
            nStakeModifier,
	...
	if (CBigNum(hashProofOfStake) > bnTarget) {
		LogPrint("DBG", "return low Hash %d \n", pindexPrev->nHeight);
		return false;
	}
    ...
    if (fDebug && !fPrintProofOfStake)
    {
        LogPrintf("CheckStakeKernelHashV2() : using modifier 0x%016x at height=%d timestamp=%s for block from timestamp=%s\n",
            ...
            DateTimeStrFormat(nStakeModifierTime),
            DateTimeStrFormat(nTimeBlockFrom));
        LogPrintf("CheckStakeKernelHashV2() : pass modifier=0x%016x nTimeBlockFrom=%u nTimeTxPrev=%u nPrevout=%u nTimeTx=%u hashProof=%s\n",
            nStakeModifier,
            nTimeBlockFrom, txPrev.nTime, prevout.n, nTimeTx,
            hashProofOfStake.ToString());
    }
...
bool CheckKernel(CBlockIndex* pindexPrev, unsigned int nBits, int64_t nTime, const COutPoint& prevout, int64_t* pBlockTime)
{
	...
	int nDepth;
	if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, nStakeMinConfirmations - 1, nDepth)) {
		LogPrint("CheckKernel", "IsConfirmedInNPrevBlocks fail\n");
		return false;
	}
	if (block.GetBlockTime() + nStakeMinAge > nTime) {
		LogPrint("CheckKernel", "fail Min Age %d, %d ,%d\n", block.GetBlockTime() , nStakeMinAge ,nTime);
		return false; // only count coins meeting min age requirement
	}
    if (pBlockTime)
