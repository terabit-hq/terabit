...
#if defined(NDEBUG)
# error "Terabit cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Terabit Signed Message:\n";
...
int64_t GetProofOfWorkReward(const CBlockIndex * pindexPrev, int64_t nFees)
{
	...
	if (pindexPrev == NULL){
		...
		nSubsidy = 800000000 * COIN;//8억* 100 = 800 억
	}else if (pindexPrev->nHeight < 200) {
		nSubsidy = 100000 * COIN;//1천만
	}
	else {
		nSubsidy = 1.6 * COIN;
	}
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (pindexPrev == NULL) {
		nSubsidy = 0;
	}else if (pindexPrev->nTime > 1514764800) {//20180101
		nSubsidy = nCoinAge * COIN_YEAR_REWARD_3 / 365; //over 20180101 reward is 0.03% until max money supplied 
	}
	else
		nSubsidy = nCoinAge * COIN_YEAR_REWARD  / 365 ;// first year reward is 0.05%
													   //limit of reward
	if (pindexPrev != NULL && (pindexPrev->nMoneySupply + nSubsidy) >= MAX_MONEY){
		LogPrintf("Max Money.... no more reward\n");
		nSubsidy = 0;
	}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	//for difficuly addjust
	if(pindexLast->nHeight<BLOCK_HEIGHT_INIT)
		return bnTargetLimit.GetCompact(); // second block
...
bool IsConfirmedInNPrevBlocks(const CTxIndex& txindex, const CBlockIndex* pindexFrom, int nMaxDepth, int& nActualDepth)
{
    for (const CBlockIndex* pindex = pindexFrom; pindex && ( pindexFrom->nHeight - pindex->nHeight )< nMaxDepth; pindex = pindex->pprev)
    {
        if (pindex->nBlockPos == txindex.pos.nBlockPos && pindex->nFile == txindex.pos.nFile)
        {
			...
			LogPrintf("IsConfirmedInNPrevBlocks():  %d,%d,%d,%d\n", pindex->nBlockPos, txindex.pos.nBlockPos, pindex->nFile, nActualDepth);
            return true;
        }
    }
    return false;
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    LogPrint("coinage", "coin age bnCentSecond:%d bnCoinDay=%s\n", bnCentSecond,bnCoinDay.ToString());
...
bool CBlock::AcceptBlock()
{
	...
	if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime, nHeight))
		return DoS(50, error("AcceptBlock() : coinbase timestamp is too early. Block Time:%d, tx[0]:%d ,FutureDrift:%d",
			GetBlockTime(), (int64_t)vtx[0].nTime, FutureDrift((int64_t)vtx[0].nTime, nHeight)));
