...
{
    // Convert terabit:// to terabit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("terabit://"))
    {
        uri.replace(0, 12, "terabit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Terabit.lnk";
}
...
{
    return GetAutostartDir() / "terabit.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Terabit-Qt") + " " + tr("version") + " " +
