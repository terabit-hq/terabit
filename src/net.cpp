...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Terabit " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Terabit is probably already running."), addrBind.ToString());
        else
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    if (!filein)
        return error("CAddrman::Read() : open failed [%s]", pathAddr.string().c_str());
